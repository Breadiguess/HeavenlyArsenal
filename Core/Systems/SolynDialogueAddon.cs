...
namespace HeavenlyArsenal.Core.Systems
{
    public class SolynDialogueAddon : ModSystem
    {
        ...
        {
            MethodInfo target = typeof(NoxusBoss.Core.Graphics.UI.SolynDialogue.SolynDialogUIManager)
        ...
        #region dialogue
        static void CommentOnBloodMoon()
        {

            var conv = DialoguePatchFactory.BuildAndRegisterFromMod(
                relativePrefix: "SolynCommentOnRiftBloodMoon",
                rootNodeKey: "Start"
            );
        ...
        static void TestNPCDialogue()
        {
            var conv3 = DialoguePatchFactory.BuildAndRegisterFromMod("SolynUmbralLeechDialogue", "Start");

            conv3.WithAppearanceCondition(c =>
            {
                int leechType = ModContent.NPCType<newLeech>();
    ...
    public static class DialoguePatchFactory
    {
        ...
        {
            // override with base 
            string filePath = $"Localization/en-US/Mods.{modName}.Solyn.hjson";
            Mod src = ModLoader.GetMod(modName) ?? throw new Exception($"Mod {modName} not found.");
            if (!src.FileExists(filePath))
                throw new Exception($"Missing localization: {filePath}");

            string raw = Encoding.UTF8.GetString(src.GetFileBytes(filePath));
            string json = HjsonValue.Parse(raw).ToString();
            ...
            string LocalizationPath = $"Mods.{modName}.Solyn.{relativePrefix}.";
            var possible = new Dictionary<string, Dialogue>();

            foreach (JToken t in doc.SelectTokens("$..*"))
            {
                if (t.HasValues) continue;
                if (t is JObject o && o.Count == 0) continue;
            ...
            FieldInfo fiTreePossible = typeof(DialogueTree).GetField("PossibleDialogue", BindingFlags.Public | BindingFlags.Instance);
            FieldInfo fiTreeRoot = typeof(DialogueTree).GetField("Root", BindingFlags.Public | BindingFlags.Instance);
            FieldInfo fiConvTree = typeof(Conversation).GetField("Tree", BindingFlags.Public | BindingFlags.Instance);
            ...
            string managerKey = $"Mods.NoxusBoss.Solyn.{relativePrefix}.";
