...
public static class WhipModifiers
{
    ...
    /// <summary>
    /// TODO: look at me again later since it was autofilled
    /// </summary>
    public class FloppyModifier : IWhipModifier
    {
        private readonly float strength;
        public FloppyModifier(float strength)
        {
            this.strength = strength;
        }
        public void Apply(List<Vector2> controlPoints, Projectile projectile, int segments, float rangeMultiplier, float progress)
        {
            ...
            for (var i = 2; i < controlPoints.Count; i++)
            {
                var prev = controlPoints[i - 1];
                var prev2 = controlPoints[i - 2];
                var dir = controlPoints[i] - prev;
                var ideal = prev + Vector2.Normalize(prev - prev2) * dir.Length();
                controlPoints[i] = Vector2.Lerp(controlPoints[i], ideal, strength);
            }
        }
    }
}
...
public static class WhipMotions
{
    ...
    /// <summary>
    /// Stolen from entropy, who (presumably) stole it from fancy whips.
    /// I feel no shame.
    /// </summary>
    public class FancyWhipMotion : IWhipMotion
    {
        public void Apply(List<Vector2> controlPoints, Projectile projectile, int segments, float rangeMultiplier, float progress)
        {
            ...
            for (var i = 0; i < segments; i++)
            {
                ...
                var vector9 = arm + (vector7 - arm) * new Vector2(1.3f, 1.3f);
    ...
    public class GrappleHandMotion : IWhipMotion
    {
        public void Apply(List<Vector2> controlPoints, Projectile projectile, int segments, float rangeMultiplier, float progress)
        {
            if (controlPoints == null)
            ...
            Vector2 BezierControlPoint = Owner.Center + Owner.DirectionTo(Main.MouseWorld) * ((projectile.ai[0] == 4 || projectile.ai[0] == 0) ? projectile.Distance(Owner.Center) / 2 : 200 * (projectile.ai[0] == 3 ? Math.Max(0f, (30f - projectile.ai[1]) / 30) : 1));

            for (int i = 0; i <= segments; i++)
            {
                float t = (float)i / segments;
                Vector2 ControlPoint = new Vector2(
                    (1 - t) * ((1 - t) * Owner.MountedCenter.X + t * BezierControlPoint.X) + t * ((1 - t) * BezierControlPoint.X + t * projectile.Center.X),
                    (1 - t) * ((1 - t) * Owner.MountedCenter.Y + t * BezierControlPoint.Y) + t * ((1 - t) * BezierControlPoint.Y + t * projectile.Center.Y)
                    );
    ...
    public class CowboyMotion : IWhipMotion
    {
        public void Apply(List<Vector2> controlPoints, Projectile projectile, int segments, float rangeMultiplier, float progress)
        {
            if (controlPoints == null)
            ...
            var origin = Main.GetPlayerArmPosition(projectile);
            controlPoints.Add(origin);
            var forward = projectile.velocity.LengthSquared() > 0.0001f ? Vector2.Normalize(projectile.velocity) : new Vector2(player.direction, 0f);
            for (var i = 1; i <= segments; i++)
            {
                var offset = forward.RotatedBy(MathHelper.ToRadians(90 * player.direction)) * (float)Math.Sin(progress * MathHelper.Pi + i) * 10f;
                controlPoints.Add(origin + forward * (rangeMultiplier * 20f * i) + offset);
            }
        }
    }
}
...
/// <summary>
/// this class is not quite the brain but still very important to the basewhip projectile.
/// it dictates what motion and modifier the whip should use.
/// </summary>
public class ModularWhipController 
{

    public readonly IWhipMotion motion;
    /// <summary>
    /// A hashset of all the <see cref="IWhipModifier"/>s that the whip may be using.
    /// </summary>
    public readonly HashSet<IWhipModifier> modifiers = new();
    ...
    public override string ToString()
    {
        ...
        msg += motion.ToString();
        msg += $"\nModifiers: ";
        foreach (IWhipModifier modifier in modifiers)
        {
            msg += $"Modifier: " + modifier.Name + $"\n";
        }
