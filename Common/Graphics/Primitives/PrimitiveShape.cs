...
namespace HeavenlyArsenal.Common.Graphics.Primitives
{
    public abstract class PrimitiveShape
    {
        ...
        public virtual void RemoveVertex(int index) => vertices.RemoveAt(index);
        public virtual void UpdateVertex(int i, Vector2 v) => vertices[i] = v;

        // Methods to add/remove edges (connecting vertices)
        public virtual void AddEdge(int indexA, int indexB) => edges.Add((indexA, indexB));
        public virtual void RemoveEdge(int index) => edges.RemoveAt(index);
