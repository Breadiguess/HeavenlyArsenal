...
internal class NightfallPlayer : ModPlayer
{
    ...
    public override void ModifyWeaponCrit(Item item, ref float crit)
    {
        ...
        var baseCrit = crit;
        var interpolant = Math.Clamp((float)DamageBucketTotal / DamageBucketMax, 0f, 1f);

        // Calculate a speed bias (slower weapons benefit more)
        var speedBias = MathF.Pow(Math.Clamp(item.useTime / 30f, 0.4f, 2.0f), 0.75f);
        // High-damage weapons gain slightly less (to prevent stacking with armor multipliers)
        var dmgBias = 1f / MathF.Pow(Math.Clamp(item.damage / 100f, 0.6f, 2f), 0.5f);

        var baseBonus = 10f;

        var bonus = interpolant * baseBonus * speedBias * dmgBias;
    ...
    public override void OnHitAnything(float x, float y, Entity victim)
    {
        ...
        {
            var a = npc.GetGlobalNPC<NightfallNPC>();
            ...
            (
                AssetDirectory.Sounds.Nightfall.Hit with
                {
                    Pitch = -0.5f + 0.1f * a.Stack,
                    Volume = 0.5f + 0.05f * a.Stack
                }
            );
        }
