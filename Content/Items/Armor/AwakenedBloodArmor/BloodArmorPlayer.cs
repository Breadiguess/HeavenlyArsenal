...
public class BloodArmorPlayer : ModPlayer
{
    ...
    public void EatClot()
    {
        var healingAmount = (int)(Player.statLifeMax2 * ClotHealingRate);
        Player.statLife += healingAmount;
        Player.AddBuff(ModContent.BuffType<BloodfinBoost>(), 360, true, true);
        CombatText.NewText(Player.Hitbox, Color.Green, healingAmount, true);
    ...
    private readonly List<SlotId> attachedSounds = new();
    ...
    private readonly float BloodHarpoonCost = 0.25f;

    /// <summary>
    ///     List of individual BloodUnits currently held.
    /// </summary>
    public List<BloodUnit> bloodUnits = new();
    ...
    private readonly float MaxResource = 1f;

    private readonly float AgingThreshold = 7f; // seconds until a blood unit converts to clot

    private readonly float ClotDrainInterval = 0.4f; // seconds between each healing tick from clot

    private readonly float ClotHealingRate = 0.015f; // fraction of max health healed per clot unit

    private float clotDrainTimer;
    ...
    private void CorrectVisuals(On_Player.orig_UpdateVisibleAccessory orig, Player self, int itemSlot, Item item, bool modded)
    {
        ...
        var hasVanityHead = !self.armor[10].IsAir;
        var hasArmorHead = self.armor[0].type == ModContent.ItemType<AwakenedBloodHelm>();
        var hasVanityBody = !self.armor[11].IsAir;
        var hasArmorBody = self.armor[1].type == ModContent.ItemType<AwakenedBloodplate>();

        if (!hasVanityHead && hasArmorHead)
        {
            var headTexture = $"AwakenedBloodHelm{actualFormStr}";
            self.head = EquipLoader.GetEquipSlot(Mod, headTexture, EquipType.Head);
        }

        if (!hasVanityBody && hasArmorBody)
        {
            var bodyTexture = $"AwakenedBloodplate{actualFormStr}";
    ...
    private void HandleOffenseForm()
    {
        ...
        if (Player.ownedProjectileCounts[ModContent.ProjectileType<BloodNeedle>()] <= TendrilCount - 1 && Main.myPlayer == Player.whoAmI)
        {
            var tentaclesPresent = new bool[TendrilCount];

            foreach (var proj in Main.projectile)
            {
                if (proj.active && proj.type == ModContent.ProjectileType<BloodNeedle>() && proj.owner == Main.myPlayer && proj.ai[1] >= 0f && proj.ai[1] < TendrilCount)
                {
            ...
            for (var i = 0; i < TendrilCount; i++)
            {
                ...
                {
                    var damage = (int)Player.GetBestClassDamage().ApplyTo(TendrilBaseDamage);
    ...
    private void AgeBlood()
    {
        ...
        var convertedThisTick = false;

        for (var i = 0; i < bloodUnits.Count && !convertedThisTick; i++)
        {
    ...
    /// </summary>
    private readonly string actualFormStr = BloodArmorForm.Defense.ToString();
    ...
    public override void ArmorSetBonusActivated()
    {
        ...
        else
        {
            ...
            if (Player.ownedProjectileCounts[ModContent.ProjectileType<BloodHarpoon>()] <= 0 && Main.myPlayer == Player.whoAmI)
            {
                if (Clot > 0)
                {
                    var cost = 0.25f;
    ...
    private void HandleFrenzy()
    {
        ...
        if (Frenzy)
        {
            ...
            {
                var buffDurationTicks = (int)(frenzyTimer * 60);
                Player.AddBuff(ModContent.BuffType<BloodArmorFrenzy>(), buffDurationTicks);
            }
    ...
    private void DrainBloodRapidly(float bloodAtTime)
    {
        ...
        var drainPerFrame = bloodAtTime / framesLeft;

        //Main.NewText($"DrainPerframe:{drainPerFrame}");
        // Drain from each unit, removing empty units
        for (var i = bloodUnits.Count - 1; i >= 0; i--)
        {
            var unit = bloodUnits[i];
            var toDrain = Math.Min(unit.Amount, drainPerFrame);
            unit.Amount -= toDrain;
    ...
    private float BloodGainChance { get; set; }
    ...
    public void AddBloodUnit(float age = 0f, float amount = 0.05f)
    {
        if (BloodArmorEquipped && !Frenzy)
        {
            var space = MaxResource - Clot - CurrentBlood;
            var toAdd = Math.Min(amount, space);
...
public class FrenzyGlowLayer : PlayerDrawLayer
{
    ...
    {
        return new BeforeParent(PlayerDrawLayers.ElectrifiedDebuffBack);
    }
    ...
    protected override void Draw(ref PlayerDrawSet drawInfo)
    {
        ...
        */
        var coronaTexture = GennedAssets.Textures.GreyscaleTextures.Corona.Asset;

        if (modPlayer.frenzyTimer > 0f)
        {
            var baseScale = 0.25f;
            var rotation = Main.GlobalTimeWrappedHourly * 2f;

            var playerCenterOnScreen = drawInfo.drawPlayer.Center - Main.screenPosition;
            ...
            var glowColor = Color.Crimson * alpha;
            var alphaByte = (byte)(255 * alpha);
            glowColor.A = alphaByte;

            // Pulsing effect
            var pulseSpeed = 5f;
            var pulse = (float)Math.Sin(Main.GlobalTimeWrappedHourly * pulseSpeed) * 0.1f;
            var finalScale = new Vector2(baseScale * (1f + pulse), baseScale * (1f + pulse));
