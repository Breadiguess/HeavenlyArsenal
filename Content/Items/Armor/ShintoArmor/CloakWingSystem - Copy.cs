...
namespace HeavenlyArsenal.Content.Items.Armor.ShintoArmor
{
    ...
    class ShintoArmorCapePlayer : ModPlayer
    {
        ...
        public DrawData GetRobeTarget() => 
            new DrawData(RobeTarget, Vector2.Zero + new Vector2(0, Player.gfxOffY), null, Color.White, -Player.fullRotation, RobeTarget.Size() * 0.5f, 1f, 0);
        ...
        private static void UpdateCloth(Player player)
        {
            if (player == null)
                return;
            ShintoArmorCapePlayer capePlayer = player.GetModPlayer<ShintoArmorCapePlayer>();
            if (capePlayer == null)
                return;
            ...
            Vector2 robePosition = player.Center + new Vector2(0, -50* player.gravDir).RotatedBy(player.fullRotation);
            robePosition += Main.OffsetsPlayerHeadgear[(int)(player.bodyFrame.Y / player.bodyFrame.Height)];
            Vector3 wind = Vector3.UnitX * (LumUtils.AperiodicSin(capePlayer.ExistenceTimer * 0.029f) * 0.67f + windSpeed) * 1.74f;
            for (int i = 0; i < steps; i++)
            {
                for (int x = 0; x < capePlayer.Robe.Width; x++)
                {
                    for (int y = 0; y < 2; y++)
