...
namespace HeavenlyArsenal.Content.Items.Armor
{
    internal class ShintoArmorIKArms : ModPlayer
    {
        ...
        internal record struct ShintoArmorLimb(IKSkeleton skeleton, bool anchored = false, bool hasTarget = false)
        {
            ...
            public void DrawArm(ref ShintoArmorLimb limb, SpriteEffects effects, int index, PlayerDrawSet drawInfo)
            {
                ...
                Rectangle frame1 = glowTexture.Frame(3, 1, 1, 0);
                DrawData a = new DrawData(glowTexture, StartPos,
                    frame1, Color.Red with { A = 0 } * limb.Opacity,
                    (limb.Skeleton.Position(0) - limb.Skeleton.Position(1)).ToRotation(), glowTexture.Frame(3, 1, 1, 0).Size() / 2 + new Vector2(30, 0), 0.9f, effects);
                ...
                Rectangle frame2 = glowTexture.Frame(3, 1, 0, 0);
                DrawData b = new DrawData(glowTexture, StartPos,
                    frame2, Color.Red with { A = 0 } * limb.Opacity,
                    (limb.Skeleton.Position(1) - limb.Skeleton.Position(2)).ToRotation(), glowTexture.Frame(3, 1, 0, 0).Size() / 2 + new Vector2(30, 0), 0.5f, effects);
                ...
                StartPos = limb.Skeleton.Position(0) - Main.screenPosition;
                DrawData c = new DrawData(armTexture, StartPos,
                    new Rectangle(94, 0, 48, 24), Color.White * limb.Opacity,
                    (limb.Skeleton.Position(0) - limb.Skeleton.Position(1)).ToRotation(), new Vector2(134 - 94, 12), 1, effects);


                drawInfo.DrawDataCache.Add(c);

                StartPos = limb.Skeleton.Position(1) - Main.screenPosition;
                DrawData e = new DrawData(armTexture, StartPos,
                    currentFrame, Color.White * limb.Opacity,
                    (limb.Skeleton.Position(1) - limb.Skeleton.Position(2)).ToRotation(), new Vector2(72, 14), 0.5f, effects);
        ...
        void UpdateLimbState(ref ShintoArmorLimb ShintoArmorLimb, Vector2 basePos, float lerpSpeed, float anchorThreshold)
        {
        ...
        public override void PostUpdateMiscEffects()
        {
            ...
            if (Active)
            {
                ...
                if (ArmsOut)
                {
                    ...
                    for (int i = 0; i < LimbCount; i++)
                    {
                        float thing = 1.5f * InverseLerp(0, 30, Player.velocity.Y);
                       
                        _limbs[i].Opacity = float.Lerp(_limbs[i].Opacity, 1, 0.1f);
                        float offset = (i % 2 == 0 ? 20 : -20) * Math.Max(thing, (1 - OpenInterpolant));
    ...
    public class ShintoArmorIKDrawLayer : PlayerDrawLayer
    {
        ...
        protected override void Draw(ref PlayerDrawSet drawInfo)
        {
            ...
            for (int i = 0; i < owner.GetModPlayer<ShintoArmorIKArms>().LimbCount; i++)
            {
