...
public abstract class CleanBaseWhip : ModProjectile
{
    ...
    protected virtual void DrawStrings(List<Vector2> points)
    {
        ...
        {
            var mid = Vector2.Lerp(points[i - 1], points[i], 0.5f);
            var color = Color.Lerp(Lighting.GetColor((int)(mid.X / 16f), (int)(mid.Y / 16f), StringColor), StringColor, Projectile.light);
    ...
    protected virtual void DrawSegs(List<Vector2> points, int FrameCount)
    {
        for (var i = 0; i < points.Count; i++)
        {
            ...
            var drawScale = Projectile.scale * GetSegScale(i, points.Count);

            var lightPos = i == 0 ? points[i] : Vector2.Lerp(points[i - 1], points[i], 0.5f);
            var color = Color.Lerp(Lighting.GetColor((int)(lightPos.X / 16f), (int)(lightPos.Y / 16f)), StringColor, Projectile.light);
            ...
            var whipTex = ModContent.Request<Texture2D>(Texture).Value;

            Main.EntitySpriteDraw
            (
                whipTex,
                points[i] - Main.screenPosition,
                new Rectangle(0, frameY, whipTex.Width, frameHeight),
                color,
                rot,
                origin,
                drawScale,
