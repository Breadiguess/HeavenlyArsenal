...
public class RoaringNight : ModProjectile
{
    ...
    public void BasicSwing()
    {
        ...
        else if (SwingStage == 1)
        {
            if (t == 0)
            {
                var basedir = Owner.direction == 1 ? MathHelper.ToRadians(-125) : MathHelper.ToRadians(230);
    ...
    {
        var Desired = Projectile.Center.AngleTo(Main.MouseWorld);
        ...
        else if (JoustProgress < 1f && Time > 40 * Projectile.MaxUpdates)
        {
            ...
            var newVelocity = Projectile.velocity * Exoblade.LungeSpeed * (0.24f + 0.76f * 0.5f);
            Owner.velocity += newVelocity;
            var rotationStrenght = MathHelper.PiOver4 * 0.05f * (float)Math.Pow(JoustProgress, 3);
            var currentRotation = Projectile.rotation;
            var idealRotation = Owner.MountedCenter.DirectionTo(Owner.Calamity().mouseWorld).ToRotation();
    ...
    public void CreateBlackGlass()
    {
        ...
        Owner.SetDummyItemTime(2);
        var Direction = Owner.Center.AngleTo(Main.MouseWorld).ToRotationVector2();
        var ToMouse = Owner.Center.AngleTo(Main.MouseWorld);

        var desiredHand = swingDirection.AngleLerp(ToMouse - MathHelper.PiOver2, 0.1f);
        ...
        var AdjustedSpawn = Owner.GetBackHandPositionImproved(Owner.compositeBackArm) + new Vector2(0, -2).RotatedBy(swingDirection);

        if (Time < placeholder && Time % 2 == 0)
        {
            var thing = Main.rand.NextFloat(100, 230);

            ScreenShakeSystem.StartShake(0.1f);

            var RotationOffset = swingDirection + MathHelper.ToRadians(Main.rand.NextFloat(-25, 26)) + MathHelper.PiOver2;
        }
        ...
        if (Time >= placeholder && Time % 30 == 0)
        {
            for (var i = 0; i < GlassCount; i++)
            {
                ...
                var Velocity = Direction.RotatedByRandom(MathHelper.ToRadians(20)) * 10;
                var Damage = Projectile.damage / 7;
                var s = Projectile.NewProjectileDirect(Owner.GetSource_FromThis(), AdjustedSpawn, Velocity, ModContent.ProjectileType<BlackGlass>(), Damage, 0);
    ...
    public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
    {
        ...
        for (var i = 0; i < 30; i++)
        {
            ...
            a.noGravity = true;
            var Variation = Main.rand.NextFloat(0.75f, 4f);
    ...
    {
        var thing = ModContent.Request<Texture2D>(Texture).Value;

        var Origin = new Vector2(thing.Width / 2, thing.Height + 60);
        var Rot = Projectile.rotation + MathHelper.PiOver2;
        var Scale = Projectile.scale * 0.2f;

        var Flip = SwingStage == 0 ? SpriteEffects.None : SpriteEffects.FlipHorizontally; // | SpriteEffects.FlipVertically;

        for (var i = 0; i < Projectile.oldPos.Length; i++)
        {
            var Afterimages = DrawPos;
        ...
        var AdjustedDrawPos = DrawPos + new Vector2(0, 10 * (1 - JoustProgress)).RotatedBy(Rot);
    ...
    public void DrawCone(Vector2 DrawPos)
    {
        ...
        float offset = 0;
        var Adjusted = Owner.GetBackHandPositionImproved(Owner.compositeBackArm) - Main.screenPosition + new Vector2(0, -2).RotatedBy(swingDirection);

        var Origin = new Vector2(0, Cone.Height / 2);

        var Rot = swingDirection + MathHelper.PiOver2;
        //DrawExtraEffects();

        var ConeColor = RainbowColorGenerator.TrailColorFunction((float)Math.Abs(Math.Sin(Main.GlobalTimeWrappedHourly + offset))) * 0.2f;
        var Squish = new Vector2(2, 1);
        Squish.Y *= (1 + MathF.Sin(Main.GlobalTimeWrappedHourly * 20) * 0.1f) * GlowSizeInterpolant;
    ...
    {
        var AdjustedSpawn = HandPos;
        var EndPos = new Vector2(0, -2);
        ...
        if (ConeVFXPos == null)
        {
            ...
            for (var i = 0; i < ConeVFXPos.Length - 1; i++)
            {
    ...
    public void DrawConeVFX(float Rotation)
    {
        if (ConeVFXPos != null && ConeVFXInterp >= 0.5f)
        {
            ...
            var HandPos = Owner.GetBackHandPosition(Player.CompositeArmStretchAmount.Full, swingDirection);
            var Origin = new Vector2(Debug.Width / 2, 0);
            var Scale = new Vector2(0.03f, 0.05f);

            for (var i = 0; i < ConeVFXPos.Length - 1; i++)
            {
                var Pos = ConeVFXPos[i] + HandPos - Main.screenPosition;
                var value = swingDirection.ToRotationVector2().AngleFrom(ConeVFXPos[i]) + MathHelper.PiOver2;
                var AdjustedColor = ConeVFXColor[i] * ConeVFXOpacity[i];
    ...
    private void DrawSlash()
    {
        ...
        var trailShader = ShaderManager.GetShader("HeavenlyArsenal.RaptureSlash");
    ...
    {
        return 70 * Projectile.scale * _slashScale * Projectile.direction;
    }
