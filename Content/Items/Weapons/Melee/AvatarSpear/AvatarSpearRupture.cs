...
public class AvatarSpearRupture : ModProjectile
{
    ...
    public override void AI()
    {
        ...
        if (Time < FlickerTime)
        {
            ...
            {
                var targetProgress = Utils.GetLerpValue(0, FlickerTime, Time, true);
        ...
        {
            var explodeProgress = Utils.GetLerpValue(0, ExplosionTime, Time - FlickerTime, true);
            ...
            if (Time == FlickerTime + 1)
            {
                ...
                SoundEngine.PlaySound
                (
                    SoundID.DD2_ExplosiveTrapExplode with
                    {
                        MaxInstances = 0
                    },
                    Projectile.Center
            ...
            var smallBlood = ModContent.GetInstance<BloodMetaball>();
            ...
            if (Time % 5 == 0)
            {
                var lightningPos = Projectile.Center + Main.rand.NextVector2Circular(20, 20);
            ...
            {
                var bombParticle = BleedingBurstParticle.pool.RequestParticle();
                var randomColor = Color.Lerp(Color.Black, Color.DarkRed, Main.rand.NextFloat());

                bombParticle.Prepare
                    (Projectile.Center + Main.rand.NextVector2Circular(35, 35), Main.rand.NextVector2Circular(35, 35), Main.rand.NextFloat(-1f, 1f), randomColor, Main.rand.NextFloat(0.5f, 1.5f));
    ...
    public override bool? Colliding(Rectangle projHitbox, Rectangle targetHitbox)
    {
        if (Time > FlickerTime)
        {
            var targetCenter = targetHitbox.Center();
            var circle = Projectile.Center + Projectile.DirectionTo(targetCenter).SafeNormalize(Vector2.Zero) * Math.Min(Projectile.Distance(targetCenter), Projectile.width);
    ...
    public override bool PreDraw(ref Color lightColor)
    {
        ...
        if (Time < FlickerTime)
        {
            var flickerScale = Projectile.scale * (0.2f + MathF.Sin(Time / 4f) * 0.1f) * (Time / FlickerTime);
