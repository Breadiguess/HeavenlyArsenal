...
internal partial class AvatarRifle_Held
{
    ...
    public float GunSwingCurveOutput => GunSwingCurve != null ? GunSwingCurve.Evaluate(t2) : 0;
    ...
    private void ManageFire()
    {
        ...
        (
            AssetDirectory.Sounds.Items.Weapons.AvatarRifle.FireSoundStrong with
            {
                Volume = 2
            },
            Owner.Center
        ...
        var Velocity = Owner.AngleTo(Owner.Calamity().mouseWorld).ToRotationVector2() * 10;

        Main.NewText(riflePlayer.ShotCount);
        var shot = Projectile.NewProjectileDirect(Projectile.GetSource_FromThis(), Projectile.Center, Velocity, bulletAMMO, Projectile.damage, Projectile.knockBack, Projectile.owner);
        var MuzzleFlash = AvatarRifle_MuzzleFlash.pool.RequestParticle();
        var tip = Projectile.Center + new Vector2(10, 0).RotatedBy(Projectile.rotation);
    ...
    private void ManageCycle()
    {
        ...
        var Rot = Projectile.rotation - MathHelper.PiOver2 - RotationOffset * LeverCurveOutput * 2;
        Rot += MathHelper.ToRadians(10);
        Owner.SetCompositeArmFront(true, Player.CompositeArmStretchAmount.Full, Rot);
    ...
    private void ManageReload()
    {
        ...
        var Rot = Owner.MountedCenter.AngleTo(Projectile.Center) - MathHelper.PiOver2;
        ...
        var BackRot = -MathHelper.ToRadians(110) * Math.Clamp(MathF.Sin(Time / 10.1f), 0.5f, 1);
