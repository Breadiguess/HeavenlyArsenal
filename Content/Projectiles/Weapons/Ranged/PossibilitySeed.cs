...
internal class PossibilitySeed : ModProjectile
{
    ...
    public void AttackTarget(NPC target)
    {
        ...
        if (Firing == 0)
        {
            ...
            var laser = Projectile.NewProjectile
            (
                Projectile.GetSource_FromThis(),
                Projectile.Center,
                Projectile.rotation.ToRotationVector2(),
                ModContent.ProjectileType<PossibilitySeed_Laser>(),
                9039,
                0f,
                Projectile.owner
            );
    ...
    public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
    {
        ...
        Projectile.Kill();
        SoundEngine.PlaySound(GennedAssets.Sounds.NPCHit.NamelessDeityHurt);
        damageDone = 0; //99% sure that this is incorrect but its 12:19 am so i dont care
    }

    public override bool PreDraw(ref Color lightColor)
    {
        var seed = TextureAssets.Projectile[Projectile.type].Value;
        var seedFrame = seed.Frame(1, 4, 1, (int)GrowthStage);
