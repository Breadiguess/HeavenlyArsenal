...
public class avatar_FishingRodProjectile : ModProjectile
{
    ...
    {
        var retracting = Projectile.timeLeft < RetractTime;
        ...
        var rodHeadPosition = Player.itemLocation + new Vector2(0, -62).RotatedBy(Player.itemRotation);
        ...
        if (Main.myPlayer == Projectile.owner)
        {
            ...
            var maxDistance = 500 * Utils.GetLerpValue(2, RetractTime, Projectile.timeLeft, true);
            ...
            var spoolOffset = new Vector2(MathF.Sin(Projectile.localAI[0] * 0.05f) * 5f, MathF.Cos(Projectile.localAI[0] * 0.025f) * 5f);
            targetPosition += spoolOffset + Player.velocity;
            var targetVelocity = (targetPosition - Projectile.Center).SafeNormalize(Vector2.Zero) * Projectile.Distance(targetPosition) * 0.8f;
        ...
        else
        {
            ...
            {
                riftPosition = Player.MountedCenter + new Vector2(0, RiftHeight);
            }
        }
    ...
    public void SetPlayerItemAnimations()
    {
        ...
        var swingTime = MathF.Pow(Time / SwingTime, 3f);
        var overShoot = 1f + MathF.Pow(Utils.GetLerpValue(SwingTime * 3, SwingTime * 1.2f, Time, true), 3) * 0.1f;
        var swingProgress = MathHelper.Lerp(swingTime, overShoot, Utils.GetLerpValue(SwingTime * 0.9f, SwingTime * 1.2f, Time, true));

        var handRotation = ((swingProgress - 1) * 3 - MathHelper.PiOver2) * Player.direction;
        var backHandRotation = handRotation + MathHelper.PiOver4 * Player.direction;
        ...
        var velocityRotation = Player.velocity.Y * 0.015f + Player.velocity.X * 0.015f * Player.direction;
        // Specifically not using AngleLerp because we want to go from negative to positive
        var targetRotation = MathHelper.Lerp(-MathHelper.PiOver2, 0.9f - velocityRotation, swingProgress) * Player.direction;
        // Using it here for some smoothing on the velocity rotation
        Player.itemRotation = Player.itemRotation.AngleLerp(targetRotation, 0.5f);
    }
    ...
    public void UpdateFishingString()
    {
        ...
        var segmentLength = 500f / 60f;
        var rodHeadPosition = Player.itemLocation + new Vector2(8 * Projectile.direction, -60).RotatedBy(Player.itemRotation);
    ...
    {
        var bellPosition = bellRope.segments[^1].position;
        var currentBellVelocity = bellRope.segments[^1].velocity;
    ...
    public void UpdateRift()
    {
        ...
        riftPosition = Vector2.Lerp(riftPosition, Player.MountedCenter + new Vector2(0, RiftHeight), 0.2f);
        var riftPositionY = riftPosition.Y - 10;

        var bell = bellRope.segments[^1];

        // Essentially creating an infinitely deep box that begins below the player
        var inBox = bell.position.Y > riftPositionY && Math.Abs(riftPosition.X - Projectile.Center.X) < 300;

        var canUse = true;
        var canDunk = (inBox && bell.velocity.Y > 0 && CurrentRiftState == (int)RiftState.Open) || CurrentRiftState == (int)RiftState.Dunking;
        ...
        if (canDunk)
        {
            ...
            if (CurrentRiftState != (int)RiftState.Dunking)
            {
                ...
                var bellSplashVelocity = new Vector2(bell.velocity.X, Math.Abs(bell.velocity.Y));

                for (var i = 0; i < 12; i++)
                {
                    Dust.NewDustPerfect
                    (
                        bell.position + Main.rand.NextVector2Circular(5, 2),
                        DustID.RedTorch,
                        bellSplashVelocity + Main.rand.NextVector2Circular(3, 1) - Vector2.UnitY * 3,
                        0,
                        Color.Red,
                        0.5f + Main.rand.NextFloat()
                    );
                    ...
                    if (!Main.rand.NextBool(5))
                    {
                        ...
                        (
                            bell.position + Main.rand.NextVector2Circular(5, 2),
                            DustID.Smoke,
                            bellSplashVelocity + Main.rand.NextVector2Circular(3, 1) - Vector2.UnitY,
                            0,
                            Color.Black,
                            1f + Main.rand.NextFloat()
                        );
                    }
                }
            }
            ...
            if (Main.myPlayer == Projectile.owner)
            {
                ...
                else
                {
                    ...
                    if (Time % waitTimeBetweenGrabs == 0)
                    {
                        ...
                        var target = Main.npc.Where(t => t.active && t.CanBeChasedBy(Player) && t.Distance(Player.MountedCenter) < maxDistance).ToArray();

                        for (var i = 0; i < target.Length; i += Main.rand.Next(1, 4))
                        {
                            var randomDirection = Main.rand.NextVector2CircularEdge(1, 1);

                            var shadowHand = Projectile.NewProjectileDirect
                            (
                                Projectile.GetSource_FromThis(),
                                target[i].Center,
                                randomDirection,
                                ModContent.ProjectileType<avatar_FishingRodVoid>(),
                                Projectile.damage,
                                0,
                                Player.whoAmI,
                                ai1: target[i].whoAmI
        ...
        if (!canUse)
        {
            ...
            SoundEngine.PlaySound
            (
                GennedAssets.Sounds.Avatar.RiftEject with
                {
                    MaxInstances = 0
                },
                bell.position
    ...
    private void Fish()
    {
        ...
        var sonarPosition = bell.position; // Bobber position as default
    ...
    public override void Load()
    {
        ...
        FishingCheck_RollDropLevels = typeof(Projectile).GetMethod("FishingCheck_RollDropLevels", BindingFlags.NonPublic | BindingFlags.Instance)
            .CreateDelegate<RollDropLevelsDelegate>(null);

        FishingCheck_ProbeForQuestFish = typeof(Projectile).GetMethod("FishingCheck_ProbeForQuestFish", BindingFlags.NonPublic | BindingFlags.Instance)
            .CreateDelegate<ProbeForQuestFishDelegate>(null);

        FishingCheck_RollEnemySpawns = typeof(Projectile).GetMethod("FishingCheck_RollEnemySpawns", BindingFlags.NonPublic | BindingFlags.Instance)
            .CreateDelegate<RollEnemySpawnsDelegate>(null);

        FishingCheck_RollItemDrop = typeof(Projectile).GetMethod("FishingCheck_RollItemDrop", BindingFlags.NonPublic | BindingFlags.Instance)
    ...
    public override bool PreDraw(ref Color lightColor)
    {
        ...
        (
            ...
            () =>
            {
                Main.spriteBatch.Begin(SpriteSortMode.Immediate, BlendState.AlphaBlend, SamplerState.AnisotropicWrap, DepthStencilState.None, Main.Rasterizer, null, Main.GameViewMatrix.EffectMatrix);

                var innerRiftTexture = AssetDirectory.Textures.VoidLake.Value;
                var edgeColor = new Color(1f, 0.06f, 0.06f);

                var dripShader = ShaderManager.GetShader("HeavenlyArsenal.AvatarRodVoidEffect");
                dripShader.TrySetParameter("time", Main.GlobalTimeWrappedHourly * 0.2f);
                dripShader.TrySetParameter("noiseScale", new Vector2(1.25f, 0.5f));
                dripShader.TrySetParameter("noiseStrength", 1.5f * riftApparitionInterpolant);
                dripShader.TrySetParameter("outlineThickness", 0.04f);
                dripShader.TrySetParameter("edgeColor", edgeColor.ToVector4());
                dripShader.SetTexture(GennedAssets.Textures.Noise.TurbulentNoise, 0, SamplerState.AnisotropicWrap);
                dripShader.SetTexture(GennedAssets.Textures.Noise.WavyBlotchNoise, 1, SamplerState.AnisotropicWrap);
                dripShader.Apply();

                Main.spriteBatch.Draw
                (
                    GennedAssets.Textures.Noise.WavyBlotchNoise,
                    new Vector2(450, RiftHeight - 10),
                    null,
                    Color.Black,
                    MathHelper.Pi,
                    new Vector2(innerRiftTexture.Width / 2, innerRiftTexture.Height),
                    new Vector2(1.33f * riftApparitionInterpolant, 0.75f * MathF.Pow(riftApparitionInterpolant, 2)),
                    0,
                    0
                );
                ...
                Main.spriteBatch.Draw(innerRiftTexture, new Vector2(450, RiftHeight), null, Color.White, -MathHelper.PiOver2, innerRiftTexture.Size() * 0.5f, new Vector2(0.4f, 2.66f), 0, 0);

                Main.spriteBatch.End();
            }
        );

        if (riftLakeTargets.TryGetTarget(Projectile.whoAmI, out var riftTarget) && riftApparitionInterpolant > 0)
        {
            var flip = Main.LocalPlayer.gravDir < 0 ? SpriteEffects.FlipVertically : 0;

            Main.EntitySpriteDraw
            (
                glow,
                riftPosition + new Vector2(0, -30) - Main.screenPosition,
                glow.Frame(),
                Color.DarkRed with
                {
                    A = 200
                } *
                riftApparitionInterpolant,
                0,
                glow.Size() * 0.5f,
            ...
            Main.EntitySpriteDraw(riftTarget, riftPosition - Main.screenPosition, riftTarget.Frame(), Color.White, 0, new Vector2(riftTarget.Width / 2, RiftHeight), 1f, flip);

            Main.EntitySpriteDraw
            (
                glow,
                riftPosition + new Vector2(0, 20) - Main.screenPosition,
                glow.Frame(),
                Color.DarkRed with
                {
                    A = 150
                ...
                0,
                glow.Size() * 0.5f,
                new Vector2(1.5f, 0.7f) * riftApparitionInterpolant,
                0
            );
            ...
            (
                glow,
                riftPosition + new Vector2(0, 14) - Main.screenPosition,
                glow.Frame(),
                Color.Black * riftApparitionInterpolant,
                0,
                glow.Size() * 0.5f,
                new Vector2(0.6f, 0.1f) * riftApparitionInterpolant,
                0
            );
        }
    ...
    {
        var itemTexture = TextureAssets.Projectile[Type].Value;
        var dir = (int)(Player.direction * Player.gravDir);
        var itemEffect = dir > 0 ? SpriteEffects.None : SpriteEffects.FlipHorizontally;
        const int upRod = 5;
        var origin = new Vector2(itemTexture.Width * (0.5f - dir * 0.5f) + upRod * dir, itemTexture.Height - upRod);

        Main.EntitySpriteDraw(itemTexture, Player.itemLocation - Main.screenPosition, itemTexture.Frame(), Color.White, Player.itemRotation - MathHelper.PiOver4 * dir, origin, 1f, itemEffect);
    }

    private void DrawSpool()
    {
        var spoolRotationOffset = MathF.Sin(Projectile.localAI[0] * 0.15f) * 0.05f;
        var spoolOrigin = new Vector2(spoolTexture.Width() / 2, spoolTexture.Height() - 10);
        Main.EntitySpriteDraw(spoolTexture.Value, Projectile.Center - Main.screenPosition, spoolTexture.Frame(), Color.White, Projectile.rotation + spoolRotationOffset, spoolOrigin, 1f, 0);
    }
    ...
    private void DrawBell()
    {
        ...
        var bellPosition = bellRope.segments[^1].position; // First index from the end
        var bellRotation = bellRope.segments[^2].position.AngleTo(bellPosition) - MathHelper.PiOver2;
        var origin = new Vector2(bellTexture.Width() / 2, 8);
        var fadeScale = Utils.GetLerpValue(SwingTime / 3, SwingTime / 2, Time, true);

        Main.EntitySpriteDraw(bellTexture.Value, bellPosition - Main.screenPosition, bellTexture.Frame(), Color.White, bellRotation, origin, Projectile.scale * fadeScale, 0);
    }
    ...
    {
        var stringTexture = TextureAssets.FishingLine.Value;

        var positions = miscRope.GetPoints();
        var stringOrigin = new Vector2(stringTexture.Width / 2, 0);

        for (var i = 0; i < positions.Length - 1; i++)
        {
            var stretch = new Vector2(1f, positions[i].Distance(positions[i + 1]) / stringTexture.Height);
            var stringRotation = positions[i].AngleTo(positions[i + 1]) - MathHelper.PiOver2;
            var stringBaseColor = Color.Lerp(new Color(255, 0, 35, 120), Color.Black, 1f - (float)i / (positions.Length - 1));
            Main.EntitySpriteDraw(stringTexture, positions[i] - Main.screenPosition, stringTexture.Frame(), stringBaseColor, stringRotation, stringOrigin, stretch, 0);
        }
    }
    ...
    private void DrawBellString()
    {
        ...
        var positions = bellRope.GetPoints();
        var stringOrigin = new Vector2(stringTexture.Width / 2, 0);
        var stringBaseColor = new Color(255, 0, 35, 120);
        var riftPosY = riftPosition.Y - 10;
        ...
        for (var i = 0; i < positions.Length - 1; i++)
        {
            ...
            var stringColor = stringBaseColor;
            ...
            Main.EntitySpriteDraw(stringTexture, positions[i] - Main.screenPosition, stringTexture.Frame(), stringColor, stringRotation, stringOrigin, stretch, 0);
        }
    }
}