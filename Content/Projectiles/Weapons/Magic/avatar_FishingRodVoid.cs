...
public class avatar_FishingRodVoid : ModProjectile
{
    public struct VoidLakeShadowHandData
    {
        ...
        public void Draw(Vector2 center, float extensionAmount, float scale, float rotation, int direction = 1)
        {
            ...
            var armTexture = AssetDirectory.Textures.VoidLakeShadowArm.Value;
            var handTexture = AssetDirectory.Textures.VoidLakeShadowHand.Value;
            var armFrame = armTexture.Frame(3, 1, ArmStyle);
            var handFrame = handTexture.Frame(3, 1, ArmStyle);
            var flip = direction > 0 ? 0 : SpriteEffects.FlipHorizontally;

            var scalingUp = 0.5f * scale * MathF.Sqrt(Utils.GetLerpValue(0, 0.5f, extensionAmount, true));
            var armScale = new Vector2(scalingUp, extensionAmount * (Length / armLength));

            var handPosition = center + rotation.ToRotationVector2() * extensionAmount * Length;
            Main.EntitySpriteDraw(handTexture, handPosition, handFrame, Color.Red, rotation + MathHelper.PiOver2, new Vector2(handFrame.Width / 2, handFrame.Height - 60), scalingUp * 1.15f, flip);

            Main.EntitySpriteDraw(armTexture, center, armFrame, Color.Red, rotation + MathHelper.PiOver2, new Vector2(armFrame.Width / 2, armFrame.Height - 16), armScale, flip);
        }
    }
    ...
    public override void AI()
    {
        ...
        {
            var attached = Main.npc[NPCToAttachTo];
        ...
        if (Time % Projectile.localNPCHitCooldown == 0 && Time < Projectile.localNPCHitCooldown * 3)
        {
            ...
            (
                GennedAssets.Sounds.NamelessDeity.IntroScreenSlice with
                {
                    PitchVariance = 0.3f,
                    Volume = 0.5f,
                    MaxInstances = 0
    ...
    public override bool PreDraw(ref Color lightColor)
    {
        ...
        var vanishTime = Utils.GetLerpValue(0, 20, Time, true) * Utils.GetLerpValue(0, 20, Projectile.timeLeft, true);
        var offset = Projectile.velocity.SafeNormalize(Vector2.Zero) * DistanceFromTarget * VisualScale;
        ...
        (
            ...
            Projectile.rotation,
            glow.Size() * 0.5f,
            new Vector2(0.12f, 0.25f) * VisualScale * vanishTime,
            0
        );
        ...
        (
            ...
            new Vector2(0.2f, 0.4f) * VisualScale,
            0,
            0
        );
        ...
        var handRot = Projectile.rotation;
        ...
        (
            ...
            Projectile.rotation,
            glow.Size() * 0.5f,
            new Vector2(0.07f, 0.2f) * VisualScale * Projectile.scale * vanishTime,
            0
        );
    ...
    private void DrawShadowHand(Vector2 center, float rotation, float scale, int index, int direction)
    {
        ...
        var extensionTime = MathF.Sqrt
            (Utils.GetLerpValue(15, 25, Time - Projectile.localNPCHitCooldown * index, true) * Utils.GetLerpValue(35, 60, Projectile.timeLeft + Projectile.localNPCHitCooldown * index, true));

        var offsetForHands = new Vector2(0, 10 * (index - 1)).RotatedBy(Projectile.rotation);
        var wobble = MathF.Sin((Main.GlobalTimeWrappedHourly * 1.67f + index * 0.2f) % 1f * MathHelper.TwoPi) * 0.03f;
