...
public class FusionRifle_Exhaust : ModProjectile, ILocalizedModType
{
    // The Fog is Coming
    ...
    public static int Fadetime => 150;
    ...
    public override void AI()
    {
        ...
        if (Time == 0f)
        {
            ...
            OrangeFogScale = Main.rand.NextFloat(0.001f, 0.1f);
            //OrangeFogRot = Main.rand.NextFloat(MathHelper.TwoPi);
            BlueFogColor.G = (byte)Main.rand.Next(120, 250 + 1);
            BlueFogScale = OrangeFogScale * Main.rand.NextFloat(0.01f, 0.1f);
        ...
        if (Time >= Fadetime)
        {
            Projectile.scale = Utils.GetLerpValue(MathHelper.Lerp(Fadetime, Lifetime, 0.1f), Fadetime, Time, true);
        ...
        var smokeRot = MathHelper.ToRadians(3f); // *Rate of rotation per frame, not a constant rotation
        var smokeColor = Color.Lerp(OrangeFogColor, BlueFogColor, 0.6f + 0.4f * MathF.Sin(Main.GlobalTimeWrappedHourly * 5f));
        Particle smoke = new HeavySmokeParticle(Projectile.Center, Projectile.velocity * 0.5f, smokeColor, 8, 0.1f * Main.rand.NextFloat(0.6f, 1.2f), 0.8f, smokeRot, required: true);
        GeneralParticleHandler.SpawnParticle(smoke);

        // Overlay the glow on top, which is on the brighter side
        if (Main.rand.NextBool(8))
        {
            var glowColor = Color.Lerp(smokeColor, Color.White, 0.25f);

            Particle smokeGlow = new HeavySmokeParticle
                (Projectile.Center, Projectile.velocity * 0.5f, glowColor, 6, Projectile.scale * Main.rand.NextFloat(0.4f, 0.7f), 0.6f, smokeRot, true, 0.005f, true);
        ...
        var lightPowerBelow = Lighting.GetColor((int)Projectile.Center.X / 16, (int)Projectile.Center.Y / 16 + 6).ToVector3().Length() / (float)Math.Sqrt(3D);
    ...
    public override bool PreDraw(ref Color lightColor)
    {
        ...
        var fog = TextureAssets.Projectile[Projectile.type].Value;
        var drawPosition = Projectile.Center - Main.screenPosition;
        var opacity = Utils.GetLerpValue(0f, 0.08f, LightPower, true) * Projectile.Opacity * 0.3f;
        Main.EntitySpriteDraw(fog, drawPosition, null, OrangeFogColor * opacity, Projectile.rotation + OrangeFogRot, fog.Size() * 0.1f, Projectile.scale * OrangeFogScale, SpriteEffects.None);
        Main.EntitySpriteDraw(fog, drawPosition, null, BlueFogColor * opacity, Projectile.rotation + BlueFogRot, fog.Size() * 0.1f, Projectile.scale * BlueFogScale, SpriteEffects.None);
