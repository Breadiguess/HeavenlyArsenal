...
partial class voidVulture
{
    ...
    public static void StandardSolynBehavior_FlyNearPlayer(BattleSolynBird solyn, NPC avatar)
    {
        ...
        var lookDestination = playerToFollow.Center;
        var hoverDestination = playerToFollow.Center + new Vector2(solynNPC.HorizontalDirectionTo(playerToFollow.Center) * -66f, -10f);

        var force = solynNPC.SafeDirectionTo(hoverDestination) * LumUtils.InverseLerp(36f, 250f, solynNPC.Distance(hoverDestination)) * 0.8f;
        ...
        var dangerCheckZone = Utils.CenteredRectangle(solynNPC.Center, Vector2.One * 450f);

        foreach (var projectile in Main.ActiveProjectiles)
        {
            var isThreat = projectile.hostile && projectile.Colliding(projectile.Hitbox, dangerCheckZone);
            ...
            var repelForceIntensity = Math.Clamp(300f / (projectile.Hitbox.Distance(solynNPC.Center) + 3f), 0f, 1.9f);
    ...
    public static void StandardSolynBehavior_AttackAvatar(BattleSolynBird solyn)
    {
        ...
        var dashPrepareTime = 10;
        var dashTime = 4;
        var waitTime = 12;
        var slowdownTime = 11;
        var wrappedTimer = solyn.AITimer % (dashPrepareTime + dashTime + waitTime + slowdownTime);

        var target = Myself is null ? AvatarRift.Myself : Myself;
        ...
        var destination = target.type == ModContent.NPCType<voidVulture>() ? target.As<voidVulture>().HeadPos : target.Center;
        ...
        else if (wrappedTimer <= dashPrepareTime + dashTime + waitTime)
        {
            ...
            if (Main.netMode != NetmodeID.MultiplayerClient && !solyn.IsMultiplayerClone)
            {
                for (var i = 0; i < 2; i++)
                {
                    var boltVelocity = Main.rand.NextVector2Circular(16f, 16f);
    ...
    public static void ShieldPlayer_Behavior(BattleSolynBird solyn, NPC npc)
    {
        ...
        var toBoss = npc.Center - solynNPC.Center;
        Vector2 dirToBoss;
        ...
        if (Myself.As<voidVulture>().Time == 0)
        {
        ...
        var desiredDistance = 86f;
        var hoverDestination = playerToProtect.Center + dirToBoss * desiredDistance + new Vector2(0f, -12f);

        // 5. Smooth steering force toward hover destination
        var toDest = solynNPC.SafeDirectionTo(hoverDestination);
        var distanceToDest = solynNPC.Distance(hoverDestination);
        var force = toDest * LumUtils.InverseLerp(24f, 300f, distanceToDest) * 0.9f;
        ...
        var shouldSpawnShield = playerToProtect.ownedProjectileCounts[ModContent.ProjectileType<DirectionalSolynForcefield3>()] < 1 && npc.As<voidVulture>().Time > 40;
        ...
        if (Main.netMode != NetmodeID.MultiplayerClient && !solyn.IsMultiplayerClone)
        {
            ...
            {
                var spawnPosition = solynNPC.Center + dirToBoss * 18f;
                var projectileRotation = dirToBoss.ToRotation();

                var projType = ModContent.ProjectileType<DirectionalSolynForcefield3>();

                // Damage 0, knockback 0, owner -1 (NPC-sourced). Pass rotation in ai0 and target npc.whoAmI in ai1.
                var Shield = Projectile.NewProjectileDirect
                (
    ...
    public static void GetImpaled_Behavior(BattleSolynBird solyn, Player player, NPC npc)
    {
        ...
        foreach (var proj in Main.ActiveProjectiles)
        {
            ...
            {
                var val = (int)(40 * (1 - LumUtils.InverseLerp(0, 400, npc.As<voidVulture>().Time))) + 4;
                ...
                if (solyn.AITimer % val == 0)
                {
                    if (Main.rand.NextBool(2))
                    {
                        ...
                        (
                            GennedAssets.Sounds.Solyn.Speak with
                            {
                                PitchVariance = 0.2f
                            }
                        );
                ...
                {
                    var a = Projectile.NewProjectileDirect(solyn.NPC.GetSource_FromThis(), solyn.NPC.Center, Vector2.Zero, ModContent.ProjectileType<SolynBarrier>(), 0, 0);
