...
namespace HeavenlyArsenal.Content.NPCs.Bosses.Fractal_Vulture.Projectiles
{
    public class NowhereGoop : ModProjectile
    {
        ...
        void HandleCollidingWithShield()
        {


            int forcefieldID = ModContent.ProjectileType<DirectionalSolynForcefield3>();
           if(DirectionalSolynForcefield3.Myself != null)
                if (DirectionalSolynForcefield3.Myself.type == forcefieldID && DirectionalSolynForcefield3.Myself.Opacity >= 0.6f && Projectile.Distance(DirectionalSolynForcefield3.Myself.Center) < 100)
                {
                    ...
                    bool hitboxCollision = DirectionalSolynForcefield3.Myself.Colliding(DirectionalSolynForcefield3.Myself.Hitbox, Projectile.Hitbox);
                    //bool reasonableIncomingAngle = DirectionalSolynForcefield3.Myself.velocity.AngleBetween(-Projectile.velocity) <= 0.23f;

                    float spinMovingAverage = DirectionalSolynForcefield3.Myself.As<DirectionalSolynForcefield>().SpinSpeedMovingAverage;
                    bool tryingToCheese = spinMovingAverage >= MathHelper.ToRadians(13f);
        ...
        public override bool OnTileCollide(Vector2 oldVelocity)
        {
            for(int i = 0; i< 12; i++)
            {
                Vector2 Vel = oldVelocity.RotatedByRandom(MathHelper.PiOver4) * 0.3f;
        ...
        public override void PostAI()
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                float size = Main.rand.NextFloat(13f, 56);
                if(Time > 60)
                    size = Main.rand.NextFloat(13f, 20 + 30 * LumUtils.InverseLerpBump(0, 60, 60, 120, Time));
                SludgeBall.CreateParticle(Projectile.Center + Main.rand.NextVector2Circular(13f, 13f) + Projectile.velocity, Projectile.velocity.RotatedBy(MathHelper.Pi).RotatedByRandom(MathHelper.ToRadians(9)) * Main.rand.NextFloat(0.3f), size);
