...
public partial class BattleSolynBird : ModNPC
{
    ...
    public override string Texture => "HeavenlyArsenal/Content/NPCs/Bosses/Fractal_Vulture/Solyn/BattleSolyn";
    ...
    public override void AI()
    {
        ...
        Invisible = false;
        BackglowScale = float.Lerp(BackglowScale, 1f, 0.02f);
        WorldMapIconScale = float.Lerp(WorldMapIconScale, 1f, 0.03f);
    ...
    public static void SummonSolynForBattle(IEntitySource spawnSource, Vector2 spawnPosition, SolynAIType state)
    {
        ...
        foreach (var npc in Main.ActiveNPCs)
        {
            if (npc.type == normalSolynID)
            {
                ...
                npc.As<BattleSolynBird>().CurrentState = state;
                npc.netUpdate = true;
    ...
    public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
    {
        ...
        if (useStaticShader)
        {
            ...
            staticShader.SetTexture(GennedAssets.Textures.Noise.WavyBlotchNoise, 2, SamplerState.PointWrap);
            staticShader.Apply();
        }
