...
namespace HeavenlyArsenal.Content.NPCs.Bosses.Fractal_Vulture
{
    ...
    partial class voidVulture//_AI
    {
        ...
        public override void AI()
        {
            ...
            {
                foreach (Player a in Main.ActivePlayers)
                    if (a.dead)
                        NPC.active = false;
            }
        ...
        public override void PostAI()
        {
            ...
            if (NPC.Opacity < 0.5f)
            {
                ...
                for (int i = NPC.buffType.Length - 1; i >= 0; i--)
                {
            ...
            if (currentState != Behavior.reveal)
            {
                ...
                {
                    voidVultureWing.UpdateWings(wing, this.NPC);
                }
                //if(!HasSecondPhaseTriggered)
        ...
        public void ManageIK()
        {
            ...
            float offset2 = MathF.Sin(Time / 10.1f + 40);
        ...
        void manageHead()
        {
            ...
            float maxStretch = Neck2.Skeleton._maxDistance * 0.86f;
            float minDistance = 38f;
            float lerpStrength = 0.6f;
            ...
            if (distance < minDistance)
            {
                ...
                Vector2 safeDir = toPlayer.SafeNormalize(Vector2.UnitY);
                Vector2 pushedTarget = NPC.Center + safeDir * minDistance;
            ...
            {
                // Regular logic
                Vector2 dir = toPlayer / distance; // faster normalize
                Vector2 idealHeadPos = NPC.Center + dir * maxStretch;
        ...
        void manageWings()
        {
            ...
            float thing = (float)wings[0].Time / voidVultureWing.WingCycleTime;//wings[0].WingCycleTime;
            if (!isFalling)
            {
