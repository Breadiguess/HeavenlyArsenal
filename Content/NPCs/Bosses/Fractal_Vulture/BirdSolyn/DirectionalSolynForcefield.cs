...
public class DirectionalSolynForcefield3 : ModProjectile, IProjOwnedByBoss<BattleSolyn>, IDrawsWithShader
{
    ...
    {
        var forcefieldShader = ShaderManager.GetShader("NoxusBoss.DirectionalSolynForcefieldShader");
        ...
        for (var i = 8; i >= 1; i--)
        {
            var afterimageRotation = Projectile.oldRot[i].AngleLerp(Projectile.rotation, 0.5f);
            var angularOffset = WrapAngle(afterimageRotation - Projectile.rotation);
            var afterimageOpacity = MathF.Exp(i * -0.4f);
            var afterimageDrawPosition = drawPosition.RotatedBy(angularOffset, Owner.Center - Main.screenPosition);
            ...
            (
                ...
                null,
                color * afterimageOpacity,
                afterimageRotation,
                GennedAssets.Textures.GreyscaleTextures.WhitePixel.Size() * 0.5f,
                Projectile.scale * Projectile.Size * 5f,
                0,
                0f
            );
        }
    ...
    {
        var width = 500; // MarsBody.GetAIInt("BrutalBarrage_SolynForcefieldWidth");
    ...
    public override void AI()
    {
        ...
        var
            width = 300 * 1; // LumUtils.InverseLerpBump(0, voidVulture.VomitCone_ShootStart, voidVulture.VomitCone_ShootStop, voidVulture.VomitCone_ShootEnd, voidVulture.Myself.As<voidVulture>().Time));
        ...
        var impactCompletionBump = LumUtils.Convert01To010(ImpactAnimationCompletion);
        var impactAFfectedScale = float.Lerp(1f, 0.6f, impactCompletionBump) + MathF.Pow(impactCompletionBump, 4f) * 0.56f;
    ...
    public void MoveTowardsMouse()
    {
        ...
        var oldDirection = Projectile.velocity.ToRotation();
        var idealDirection = Solyn.AngleTo(voidVulture.Myself.Center);

        var reorientInterpolant = (1f - DisappearanceInterpolant) * (1f - GameSceneSlowdownSystem.SlowdownInterpolant);
        var newDirection = oldDirection.AngleLerp(idealDirection, reorientInterpolant * 0.25f).AngleTowards(idealDirection, reorientInterpolant * 0.008f);
        ...
        var angleDifference = WrapAngle(newDirection - oldDirection);
        SpinSpeedMovingAverage = float.Lerp(SpinSpeedMovingAverage, Math.Abs(angleDifference), 0.03f);

        var offset = float.Lerp(90f, 67f, LumUtils.Convert01To010(ImpactAnimationCompletion));
    ...
    {
        return Projectile.RotatingHitboxCollision(targetHitbox.TopLeft(), targetHitbox.Size()) && Projectile.Opacity >= 0.56f;
    }
