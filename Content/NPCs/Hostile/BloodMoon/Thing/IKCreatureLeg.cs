    ...
    class IKCreatureLeg
    {
        ...
        public IKCreatureLeg(NPC Owner, int i, IKCreatureLeg sister)
        {
            ...
            Skeleton = new IKSkeleton(
                        ...
                        {
                            MinAngle = i % 2 == 0 ? MathHelper.ToRadians(20)
        ...
        public void UpdateLimbState(ref IKCreatureLeg IKCreatureLeg, Vector2 basePos, float lerpSpeed, float anchorThreshold, int i)
        {
            ...
            {
                float bell = Convert01To010(IKCreatureLeg.StepProgress);//(float)Math.Sin((1 - IKCreatureLeg.StepProgress) * MathHelper.Pi);
            ...
            float maxDist = IKCreatureLeg.Skeleton._maxDistance * 0.9f;
            if (IKCreatureLeg.GrabPosition.HasValue)
        ...
        public Vector2 FindNewGrabPoint(Vector2 basePos, int index)
        {
            ...
            float maxDist = Creature.As<IkCreature>().Legs[index].Skeleton._maxDistance * 0.65f;

            float sideOffset = Math.Clamp(Creature.velocity.X * 40f, -100, 100);
        ...
        bool IsLimbGrounded(IKCreatureLeg limb)
        {
        ...
        public void Draw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
        {
            ...
            for (int i = 0; i < Skeleton.PositionCount - 1; i++)
            {
