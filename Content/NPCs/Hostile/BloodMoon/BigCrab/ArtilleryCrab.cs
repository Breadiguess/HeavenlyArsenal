...
internal class ArtilleryCrab : BaseBloodMoonNPC
{
    ...
    {
        var distance = Vector2.Distance(NPC.Center, target.Center);
        ...
        for (var i = 0; i < Main.maxNPCs; i++)
        {
            ...
            if (other.active && other.whoAmI != NPC.whoAmI && other.type == NPC.type)
            {
                ...
                if (dist < pushRadius && dist > 0f)
                {
                    ...
                    var pushAmount = (pushRadius - dist) / pushRadius;
    ...
    private void DoEvicerate(Player target)
    {
        ...
        var dist = target.Center - NPC.Center;
    ...
    {
        var ChargeDistance = NPC.Center.X - target.Center.X;
        var pushRadius = 40f;
        var pushStrength = 13f;
        ...
        if (Time <= 20 && Time % 6 == 0)
        {
            for (var i = 0; i < 2; i++)
            {
                var dir = (target.Center - NPC.Center).SafeNormalize(new Vector2(0, i * 15 - 15)) * 10f;
    ...
    {
        var angle = MathHelper.ToRadians(75f); // steep 75° arc
        const float v0 = LaunchSpeed;

        var direction = targetPos - NPC.Center;
        var distance = direction.Length();
        var vx = v0 * (float)Math.Cos(angle);
        var vy = v0 * (float)Math.Sin(angle);

        // Scale vx to cover horizontal distance
        vx = vx * (distance / (vx * (2 * vy / Gravity)));

        var launchV = new Vector2
        (
    ...
    {
        var dir = Math.Sign(pos.X - NPC.Center.X);
    ...
    {
        var dir = Math.Sign(NPC.Center.X - pos.X);
        NPC.velocity.X = dir * speed;
    ...
    {
        var dirX = Math.Sign(pos.X - NPC.Center.X);
    ...
    private void createGore(int i)
    {
        ...
        //thanks lucille
        GetGoreInfo(out _, out var goreID, i);
...
public class BloodMortar : ModNPC
{
    ...
    {
        var texture = ModContent.Request<Texture2D>("HeavenlyArsenal/Content/NPCs/Hostile/BloodMoon/BigCrab/BloodMortar").Value;
