...
public partial class newLeech
{
    ...
    {
        var temp2 = Main.player[NPC.FindClosestPlayer()];

        var temp = new List<NPC>(Main.npc.Length);
        ...
        currentTarget = null;
        var thing = Main.rand.NextFloat(0, 1);
    ...
    private void ManageLunge()
    {
        ...
        if (Time < 120f)
        {
            ...
            if (currentTarget != null && currentTarget.active)
            {
                var toTarget = currentTarget.Center - NPC.Center;
                var distance = toTarget.Length();
                var dir = distance > 0.01f ? toTarget / distance : Vector2.Zero;

                // Calculate a point beyond the player for overshoot
                var overshootDistance = 60f; // How far past the player to aim
                var overshootTarget = currentTarget.Center + dir * overshootDistance;

                var toOvershoot = overshootTarget - NPC.Center;
                var desiredVelocity = Vector2.Normalize(toOvershoot) * 20f * accelerationInterp;

                // Add a small random angle to the dash for less predictability
                var randomAngle = Main.rand.NextFloat(-0.15f, 0.15f);
    ...
    private void ManageFlee()
    {
        ...
        var difference = NPC.Center.AngleFrom(currentTarget.Center).ToRotationVector2() * 10;
    ...
    private void DisipateIntoBlood(bool hasused)
    {
        if (currentTarget != null)
        {
    ...
    private void DoDeathAnimation()
    {
        ...
        if (Time == 1 && Main.netMode != NetmodeID.Server)
        {
            SoundEngine.PlaySound
            (
                AssetDirectory.Sounds.NPCs.Hostile.BloodMoon.UmbralLeech.DyingNoise with
                {
                    MaxInstances = 1
                }
            );
        ...
        if (Time > DeathTime)
        {
            for (var u = 0; u < AdjHitboxes.Length; u++)
            {
                createGore(AdjHitboxes[u].Center(), u);

                for (var i = 0; i < 4; i++)
                {
                    var bloodSpawnPosition = AdjHitboxes[u].Center(); //Main.npc[Segments[u].whoAmI].Center;
            ...
            SoundEngine.PlaySound
            (
                new SoundStyle("HeavenlyArsenal/Assets/Sounds/NPCs/Hostile/BloodMoon/UmbralLeech/Death", 3) with
                {
                    MaxInstances = 0
                },
                NPC.Center
        ...
        else if (Time < DeathTime - 20)
        {
            if (Time % 6 == 0)
            {
                for (var u = 0; u < AdjHitboxes.Length; u++)
                {
                    for (var i = 0; i < 4; i++)
                    {
                        var bloodSpawnPosition = AdjHitboxes[u].Center();
                        var bloodVelocity = (Main.rand.NextVector2Circular(30f, 30f) - NPC.velocity) * Main.rand.NextFloat(0.2f, 1.0f);
