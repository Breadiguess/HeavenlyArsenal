        ...
        */
        ...
        public override void AI()
        {
            ...
            if (SegmentNum >= 1f)
            {
                ...
                UmbralLeech head = (UmbralLeech)Main.npc[headIndex].ModNPC;
            ...
            if (NPC.life <= 2)
            {
                ...
                for (int i = 0; i < Main.maxNPCs; i++)
                {
                    ...
                    if (npc.active && npc.type == thisType)
                    {
                        ...
                        {
                            if (npc.ModNPC is UmbralLeech leech)
                            {
        ...
        private void DoDeathAnimation()
        {
            ...
            if (DeathAnimationTimer >= 220f)
            {
                ...
                NPC.HitEffect();
                NPC.NPCLoot();
                //Main.NewText($"{NPC.GetSource_Loot("")}");
                NPC.active = false;
        ...
        public override void SetBestiary(BestiaryDatabase database, BestiaryEntry bestiaryEntry)
        {
            ...
            {
                new FlavorTextBestiaryInfoElement("A massive leech that thrives during blood moons. It propels itself by rapidly contracting its body, allowing it to move with surprising speed and agility. It hunts by sensing the heat and movement of its prey, latching onto them with its powerful jaws to feed."),
        ...
        {
            if (Main.bloodMoon){
                if (NPC.downedMoonlord && !DownedBossSystem.downedDoG)
                {
                    return SpawnCondition.OverworldNightMonster.Chance * 0.074f;
                }
