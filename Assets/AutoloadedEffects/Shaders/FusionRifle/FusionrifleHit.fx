...
float4 PixelShaderFunction(float4 sampleColor : COLOR0, float2 coords : TEXCOORD0) : COLOR0
{
    ...
    float4 blurColor = GaussianBlur(coords, convertFromScreenCoords(2).x, 2);
    
    // Used to negate the need for an inverted if (baseColor.a > 0) by ensuring all of the edge checks fail.
    float alphaOffset = (0.1f - any(baseColor.a));
    ...
    float4 localEdgeColor = lerp(edgeColor, edgeColor, sin(length(worldUV / screenSize) * 25) * 0.5 + 0.5);
